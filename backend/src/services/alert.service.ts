...
export async function evaluateAndCreateAlert(input: AlertInput): Promise<void> {
  ...
  const shouldAlert =
    leakProb >= config.alerts.leakThreshold ||
    severity_pct >= config.alerts.severityThreshold;

  if (!shouldAlert) return;

  // Determine alert type
  let alertType: "minor_leak" | "major_leak" | "anomaly" | "pressure_spike";
  let message: string;
  ...
  if (existing && existing.length > 0) {
    logger.debug("Skipping duplicate alert", { pipe_id, alertType });
    return;
  }
  ...
  if (error) {
    logger.error("Failed to create alert", { error: error.message, pipe_id });
  } else {
    logger.info("Alert created", { pipe_id, alertType, severity_pct });
